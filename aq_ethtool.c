...
#ifdef __VMKLNX__
u32 aq_ethtool_get_rx_csum(struct net_device *ndev)
{
	return (ndev->features & NETIF_F_RXCSUM) ? 1 : 0;
}

int aq_ethtool_set_rx_csum(struct net_device *ndev, u32 rx_csum)
{
	struct aq_nic_s *aq_nic = netdev_priv(ndev);
	struct aq_nic_cfg_s *aq_nic_cfg = aq_nic_get_cfg(aq_nic);
	int err = 0;
	...
	if ((ndev->features ^ aq_nic_cfg->features) & NETIF_F_RXCSUM) {
		aq_nic_cfg->features = ndev->features;
...
#endif /*__VMKLNX__*/
#ifdef __VMKLNX__
	.get_rx_csum         = aq_ethtool_get_rx_csum,
	.set_rx_csum         = aq_ethtool_set_rx_csum,
