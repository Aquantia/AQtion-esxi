...
#endif
#ifdef __VMKLNX__
	self->ndev->real_num_tx_queues = 1;
	self->ndev->watchdog_timeo = 5 * HZ;
#endif
}
...
#ifdef __VMKLNX__
	/*In that case we can get SKB with 1st fragment larger
	 * than AQ_CFG_TX_FRAME_MAX. Don't worry about map/unmap_single
	 * map/unmap_page because single works the same as page
	 */

	if(dx_buff->len > AQ_CFG_TX_FRAME_MAX) {
		unsigned int frag_size = dx_buff->len;
		dma_addr_t frag_pa = dx_buff->pa;
		dx_buff->len = AQ_CFG_TX_FRAME_MAX;
		frag_size -= dx_buff->len;
		frag_pa += AQ_CFG_TX_FRAME_MAX;

		while(frag_size) {
			unsigned int buff_size = 0U;
			if(frag_size > AQ_CFG_TX_FRAME_MAX) {
